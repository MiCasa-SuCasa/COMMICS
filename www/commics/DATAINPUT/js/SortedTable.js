...
SortedTable.selectAll = function(elm) {
	var st = SortedTable.getSortedTable(elm);
	st.selectall(elm);
}
SortedTable.cleanAll = function(elm) {
	var st = SortedTable.getSortedTable(elm);
	st.cleanselect();
}
SortedTable.SelectedItems = function(elm) {
	var st = SortedTable.getSortedTable(elm);
	return st.selectedItems();
}
	...
	callBodyClick:function(e) {
		...
		if (e.shiftKey && st.allowMultiple) st.selectRange(elm);
			...
			if (st.selected(elm)) {
				...
				if ( !st.allowMultiple) st.cleanselect(); //!e.ctrlKey ||
				st.select(elm);
			}
		}
		return false;
...
// inited
	prepHeader:function() {
		var ths = this.head.getElementsByTagName('th');
                var tbs = this.body.getElementsByTagName('th');
		for (var i=0;i<ths.length;i++) {
	...
	prepBody:function() {
		...
		for (var i=0;i<trs.length;i++) {
			if (this.hasClass(trs[i],'nosort')) continue;
...
if ( ! this.hasClass(elem,'nosort') )
{		this.selectedElements.push(elem);
		if (typeof(this.onselect)=='function') this.onselect(elem);
		this.changeClass(elem,'','selrow');
}
		elem = elem.nextSibling;
		while(elem) {
			if( elem.nodeType  == 1 &&  ! this.hasClass(elem,'nosort') )
			{
				this.selectedElements.push(elem);
				if (typeof(this.onselect)=='function') this.onselect(elem);
				this.changeClass(elem,'','selrow');
...
// movement
	...
	selectedItems:function() {
		var string2return = '';
		for (var i=0;i<this.selectedElements.length;i++) {
			string2return += this.selectedElements[i].cells.item(0).firstChild.firstChild.nodeValue; //inside the td we have a <a> element
			if( i < this.selectedElements.length-1 ){string2return +='-';}
		}
